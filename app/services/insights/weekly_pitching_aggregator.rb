module Insights
  ...
  # 相関の「成績側」変数（投手）として使う。ERA 等は generator と同じ *9 基準。
    ...
    def pitching_rows
      ...
      @user.game_results.joins(:match_result, :pitching_result)
        ...
        { date: row[0], innings_pitched: row[1].to_f, earned_run: row[2].to_f,
          hits_allowed: row[3].to_f, base_on_balls: row[4].to_f }
      end
    end
  end
end
